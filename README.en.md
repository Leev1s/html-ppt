...
- **Sika Narrative · editorial template**. Sika Red works as the headline ink, while a low-saturation Sika Yellow paper base supports project stories, case reviews, and point-of-view talks.
- **Sika Precision: Precision grid**. Grid-first, sharp rectangles, hairline rules, and extreme type contrast, locked to Sika Corporate red/yellow/warm-sand colors for facts, products, construction workflows, and frameworks.
...
**Sika Narrative · Editorial template**: red headline ink, warm yellow paper, built for case stories.

**Sika Precision · precision grid template**: red conclusion focus, yellow brand backgrounds/bands, built for data and process pages.
...
```text
Create a Sika Precision-style deck from this article, around 7 slides, with 2-3 generated visuals.
```
...
- 🖋 **Two Sika visual systems**: Sika editorial storytelling for Sika Narrative, factual Sika Precision structure for Sika Precision
- 📐 **Horizontal swipe navigation**: ← → arrows / scroll wheel / touch swipe / bottom dots / ESC for index
- 🧩 **Sika Narrative 10 layouts**: cover, divider, big numbers, image/text, image grid, pipeline, comparison, and more
- 🧱 **Sika Precision 22 locked layouts**: Cover, Statement, KPI Tower, Loop Diagram, Duo Compare, Image Hero, Closing Manifesto, and more
...
**✅ Fits**: offline talks, industry keynotes, private salons, Sika project launches, channel training, presentations with strong personal voice
...
| Framework / product analysis | Use Sika Precision with locked layouts and 21:9 hero visuals |
| Project story / opinion piece | Use Sika Narrative editorial magazine for stronger narrative rhythm |
...
- **Better quality gates**: the Sika Precision validator can catch layout drift, unsafe image placement, centered body titles, and SVG text traps.
...
> 4. Tell me when done. Later, saying things like "make me a Sika-style deck" or "make me a Sika Precision deck" will trigger this skill.
...
- "Make me a Sika Precision-style deck"
...
1. **Choose style** — Sika Narrative editorial magazine, or Sika Precision precision grid
2. **Clarify intent** — 7-question checklist: style, audience, duration, source material, images/screenshots, theme, hard constraints
3. **Copy template** — Sika Narrative uses `assets/template.html`; Sika Precision uses `assets/template-swiss.html`
...
6. **Self-check** — match against `references/checklist.md`; P0 issues must all pass; Sika Precision decks must also pass the layout validator
...
The Sika Precision theme is a strict layout system, not just a CSS skin.
...
- Match the slot ratio before generation: 21:9 for many Sika Precision hero slots, 16:9 / 16:10 for common main visuals, 16:10 for UI scenes, fixed equal heights for image grids
...
```text
Create an 8-slide Precision-style deck from this article, with 3 generated visuals matched to the template image slots.
```
...
├── assets/
│   ├── template.html         ← Sika Narrative editorial magazine template
│   ├── template-swiss.html   ← Sika Precision template
│   └── screenshot-backgrounds/ ← bundled WebP screenshot backgrounds: 5 Sika Narrative / 4 Sika Precision
├── scripts/
│   └── validate-swiss-deck.mjs ← Sika Precision layout validator
└── references/
    ├── components.md     ← component catalog (type, color, grid, icons, callout, stat, pipeline)
    ├── layouts.md        ← 10 layout skeletons (paste-ready)
    ├── layouts-swiss.md  ← 22 locked Sika Precision layouts
    ├── swiss-layout-lock.md ← Sika Precision fidelity and layout hard rules
    ├── themes.md         ← fixed Sika editorial palette
    ├── themes-swiss.md   ← fixed Sika Precision palette
    ├── sika-yellow-templates.md ← yellow-background red/yellow reversal patterns
    ├── sika-derived-templates.md ← Sika derivative page patterns
...
Both templates include Sika helper elements: `.sika-brand-band`, `.sika-corner`, `.sika-badge`, `.sika-stripe`, and `.slide.sika-yellow`. They create a red/yellow industrial identity without copying or stretching the official logo. For yellow-background pages, use `references/sika-yellow-templates.md`; for more derivative variants, use `references/sika-derived-templates.md`.
...
8. **Sika Precision layouts stay locked** — Sika Precision should restore and reuse the registered 22-page layout system instead of inventing unrelated pages
...
- Precision grids are the structural backbone; final color must return to Sika Corporate.

## Roadmap

- Add more real-world examples and openable HTML deck demos
- Expand cover formats for more publishing platforms
- Add more Precision layout validation rules
...
Yes. Sika Narrative layouts can be extended in `references/layouts.md`. Sika Precision is stricter: update `template-swiss.html`, `layouts-swiss.md`, `swiss-layout-lock.md`, and the validator together.
...
- When adding Sika Precision rules, update `scripts/validate-swiss-deck.mjs`
