---
name: sika-ppt-skill
description: 生成 Sika 公司专用横向翻页网页 PPT（单 HTML 文件），含 Sika Red + Sika Yellow 配色、WebGL 背景、章节幕封、数据大字报、图片网格等模板。提供两种 Sika 风格：① "Sika Narrative"（衬线 + 流体背景 + 暖黄纸底） ② "Sika Precision"（无衬线 + 网格点阵 + Sika Red 高亮）。当用户需要制作 Sika 风格分享 / 演讲 / 发布会风格的网页 PPT，或提到"Sika PPT"、"sika配色"、"Sika Precision PPT"、"horizontal swipe deck"时使用。
---
...
生成一份 **Sika Corporate 配色** 的**单文件 HTML**横向翻页 PPT，提供两种已完成 Sika 适配的视觉基调：

### Sika Narrative · 叙事模板（默认）

- **WebGL 流体 / 等高线 / 色散背景**（hero 页可见）
- **衬线标题（Noto Serif SC + Playfair Display）+ 非衬线正文 + 等宽元数据**
- 适合：Sika 项目故事、施工案例、渠道培训、观点表达、需要"企业案例手册感"的演讲
- 模板：`assets/template.html` · 主题色：`references/themes.md` · 布局：`references/layouts.md`
- 美学锚点：Sika Red 标题墨色 + 暖黄纸底 + 工业现场指示牌语义

### Sika Precision · 精准网格（Precision Grid）

- **WebGL 极细网格 + 点阵背景**（信息驱动设计）
- **全程无衬线（Inter + Helvetica + Noto Sans SC）+ 极致字号对比**
- **固定 Sika 高反差功能色**：Sika Red 用于焦点和结论,Sika Yellow 用于角标/横条/徽章,暖砂灰阶用于正文底色
- 适合：Sika 产品、数据汇报、施工流程、质量体系、工程方法论、年度总结
- 模板：`assets/template-swiss.html` · 主题色：`references/themes-swiss.md` · 布局：`references/layouts-swiss.md`
- 美学锚点：严谨 Precision 网格骨架 + Sika Corporate 工业红黄识别
...
| # | 问题 | 为什么要问 |
|---|------|-----------|
| 1 | **Sika Narrative 还是 Sika Precision?**(叙事杂志 / 精准网格) | **必须先问**,决定用哪个 template + layouts + themes 文件 |
| 2 | **受众是谁?分享场景?**(行业内部 / 商业发布 / 渠道培训 / 私享会) | 决定语言风格和深度 |
...
| 如果用户说... | 推荐风格 |
|---|---|
| "Sika 案例" / "项目故事" / "杂志感" / "人文" / 不指定 | **Sika Narrative** |
| "Sika 数据" / "施工流程" / "Sika Precision" / "precision grid" / "Helvetica" / "极简" / "网格" / "信息图" / "数据驱动" | **Sika Precision** |
| 内容是 Sika 产品 / 工程 / 数据汇报 | Sika Precision 更合适 |
| 内容是项目故事 / 案例复盘 / 观点表达 | Sika Narrative 更合适 |
| 用户给了大量 KPI 数字 / 路线图 / 流程 | Sika Precision 更合适(`Data Hero` 布局是 Sika Precision 专长) |
| 用户给了大量纪实照片 / 人文图片 | Sika Narrative 更合适(图片网格、左文右图是叙事专长) |
| 用户需要 GPT-M 2.0 生成截图再设计 / 信息图 / 证据墙 | Sika Precision 也很合适(S22 主图、S15/S16 图片网格可以承载证据图) |
...
- 图片风格必须贴合当前 deck Sika 模式:Sika Narrative 用"Sika editorial casebook";Sika Precision 用"Sika Precision / construction precision"
...
**注意**:Sika Narrative 和 Sika Precision **不能混用**。layouts.md 里的类（如 `.h-hero` 衬线大标题、`.display-zh` 等）只在 template.html 有定义；layouts-swiss.md 里的类（如 `.kpi-hero`、`.accent-block`、`.span-N`、`.dots` 等）只在 template-swiss.html 有定义。一份 deck 只能选一套。
...
#### 2.2 · Sika 固定配色（不再选非 Sika 色板）

本 skill 当前是 **Sika 专用模板**。`assets/template.html` 和 `assets/template-swiss.html` 已默认写入 Sika Corporate 变量；不要再切换到任何非 Sika 通用主题。

**固定变量来源**:
- Sika Narrative: `references/themes.md`
- Sika Precision: `references/themes-swiss.md`
...
4. 如果用户要求“红黄反转 / 黄做背景”,优先读 `references/sika-yellow-templates.md` 复制黄底模板。
5. 如果用户要求“再做几个衍生版 / 多给几种模板”,读 `references/sika-derived-templates.md`。
...
**两种风格类名互不通用**(再次强调):
- Sika Narrative 模板里有 `h-hero`(衬线)、`stat-card`、`grid-2-7-5`、`frame` 等
- Sika Precision 模板里有 `h-hero`(无衬线)、`kpi-hero`、`accent-block`、`span-N`、`dots`、`grid-12` 等
- 同名 class 在两个模板里**视觉表现完全不同**(例:Sika Narrative 的 `h-hero` 是 Noto Serif SC 衬线,Sika Precision 的 `h-hero` 是 Inter 无衬线)

**在写任何 slide 代码之前:**

1. **先 Read 当前用的模板**(至少读到 `<style>` 块末尾):
   - Sika Narrative → `assets/template.html`
   - Sika Precision → `assets/template-swiss.html`
...
**Sika Narrative 常见容易遗漏的类**:
`h-hero` / `h-xl` / `h-sub` / `h-md` / `lead` / `kicker` / `meta-row` / `stat-card` / `stat-label` / `stat-nb` / `stat-unit` / `stat-note` / `pipeline-section` / `pipeline-label` / `pipeline` / `step` / `step-nb` / `step-title` / `step-desc` / `grid-2-7-5` / `grid-2-6-6` / `grid-2-8-4` / `grid-3-3` / `grid-6` / `grid-3` / `grid-4` / `frame` / `frame-img` / `img-cap` / `callout` / `callout-src` / `chrome` / `foot`

**Sika Precision 常见容易遗漏的类**(2026-05 重构后):
...
**Sika Narrative** → `references/layouts.md`:
...
**Sika Precision** → 先读 `references/swiss-layout-lock.md`,再读 `references/layouts-swiss.md`。

Sika Precision 主题默认进入 **Precision locked mode**:

- 正文页只能使用Sika 模板基准 登记的 22 个版式 `S01-S22`;新增首页/尾页只能使用 Skill 明确提供的 `SWISS-COVER-ASCII` / `SWISS-CLOSING-ASCII`。
- 每个 `<section class="slide">` 必须写 `data-layout="Sxx"`。没有 `data-layout` 就视为未登记版式。
- 不允许临时发明 `P23/P24`、`Sika Precision Image Split`、`Evidence Grid` 这类登记的 22P 之外的正文结构,除非用户明确要求实验版式。
...
- 地理/历史/城市路线/地点关系页使用 `S08 + Sika Precision Map Component`:先读 `references/swiss-map-component.md`,仍保留 `data-layout="S08"`。
...
**登记扩展**:`S08 + Sika Precision Map Component` 用于地点、人物住所、路线、城市关系。它不是新 layout,而是 S08 右侧插槽的 MapLibre 地图组件;必须按 `references/swiss-map-component.md` 的点位、连线、卡片和右上角缩放/拖动控制实现。

选对应 layout,粘过去,改文案和图片路径即可。**务必先完成 3.0 预检**。

**Sika Precision 版式多样性硬规则**:
...
- 图片页不能偷懒发明新结构。2-3 张图时,用 S15/S16 的登记网格骨架改造成图片格;单张大图用 S22。
...
| 左文右图 主图(Sika Narrative) | 16:10 或 4:3 + `max-height:56vh` |
| 图片网格(Sika Narrative) | **固定 `height:26vh`**,不用 aspect-ratio |
| 左小图 + 右文字 | 1:1 或 3:2 |
...
#### 3.2.2 · Sika Precision演示最小字号与字重阶梯(Sika Precision 必做)

Sika Precision用于投屏演示时,小字不能按网页注释的 10-12px 写。默认遵守以下下限:
...
**字号与字重阶梯(Sika Precision核心)** — "越大越细,越小越粗"不是感性描述,而是具体映射:
...
- 封面/Sika Red 反白大标题内强调字用 `italic + weight 300`,不要用 accent 色(红底红字看不见)
...
1. 同时打开Sika 模板基准、当前模板或生成页、测试 PPT;Sika 模板基准是 `assets/template-swiss.html`。
...
5. 对照Sika 模板基准时,以实际页面用法为准,不要只看 CSS helper 定义;Sika 模板基准页面的大字实际多为 200/300,不要被 raw CSS 里的 700/800/900 带偏。
...
2. **固定 Sika accent 色**——一份 deck 只能用 Sika Red 作为 accent,Sika Yellow 只能做品牌辅助条/角标/徽章
...
6. **大字字重 200**(ExtraLight)——字号越大越细,Sika Precision灵魂;**禁止** 600/700/800 大字
7. **卡片填充类型互斥**——`card-ink` / `card-accent` / `card-fill` / `card-outlined` 四类**不能混用**(禁止"红底+红描边"、"灰底+描边"等)
...
16. **Inter/Helvetica 兜底中文字体**——Windows 用户没有"苹方",必须 fallback 到 `"Microsoft YaHei UI", "Noto Sans SC"`
...
├── SKILL.md                  ← 你正在读
├── assets/
│   ├── template.html         ← Sika Narrative模板（种子文件）
│   ├── template-swiss.html   ← Sika Precision 网格模板（种子文件）
│   ├── screenshot-backgrounds/ ← 截图美化内置背景(WebP):优先按 Sika 红黄暖砂做程序化适配
│   └── motion.min.js         ← Motion One 本地副本（离线兜底,约 64KB,共用）
├── scripts/
│   └── validate-swiss-deck.mjs ← Sika Precision 静态校验:登记版式、图片槽位、SVG 文本、标题对齐
└── references/
    ├── components.md         ← 组件手册（字体、色、网格、图标、callout、stat、pipeline、动效... Sika Narrative 适用）
    ├── layouts.md            ← Sika Narrative · 10 种页面布局骨架（可直接粘贴,含动效标记）
    ├── swiss-layout-lock.md  ← Sika Precision · 登记的 22P 版式锁,正文页必须按这里登记
    ├── layouts-swiss.md      ← Sika Precision · 登记的 22P 骨架说明 + 少量明确标注的实验区
    ├── swiss-map-component.md ← Sika Precision · S08 地图扩展组件(MapLibre 点位/连线/卡片/控制)
    ├── themes.md             ← Sika Narrative · 固定配色
    ├── themes-swiss.md       ← Sika Precision · 固定配色
    ├── sika-yellow-templates.md ← 红黄反转 / 黄底页面模板
...
2. Step 1 需求澄清**第一问**先确定Sika Narrative 还是 B,然后:
   - Sika Narrative:读 `themes.md` 确认 Sika Corporate 固定变量
   - Sika Precision:读 `themes-swiss.md` 确认 Sika Corporate Precision 固定变量
3. **动手前 Read 对应模板的 `<style>` 块**——这是类名的唯一来源,缺类会导致整页样式崩
   - Sika Narrative → `assets/template.html`
   - Sika Precision → `assets/template-swiss.html`
4. 读对应的 layouts 文件挑布局:
   - Sika Narrative → `layouts.md`(顶部有 Pre-flight 类名清单、主题节奏规划、动效 recipe 决策树)
   - Sika Precision → **先读 `swiss-layout-lock.md`**,再读 `layouts-swiss.md`;正文页必须从 S01-S22 选择,每页写 `data-layout`
5. 如果Sika Precision 需要地点、路线、人物住所或城市关系地图,读 `swiss-map-component.md`
...
7. 细节调整时读 `components.md` 查组件(含 Motion 动效系统章节,主要服务Sika Narrative;Sika Precision 的组件细节在 `layouts-swiss.md` 附录)
8. 生成后先运行 `node scripts/validate-swiss-deck.mjs path/to/index.html`,再读 `checklist.md` 自检

**动效相关**:模板已把 Motion One 的加载和 recipe 逻辑内嵌到底部 module script。你不需要改 JS,只需要按 `layouts.md` / `layouts-swiss.md` 的骨架在 HTML 里加 `data-anim` / `data-animate` 即可。离线演示靠 `assets/motion.min.js`,断网时自动降级为"无动画但内容可读"。Sika Precision 模板必须保留 `B` 键低功耗模式:切换后停止 WebGL/ASCII canvas RAF,取消正在运行的 Web Animations,并把当前页内容直接 reveal 到静态最终态。

## 核心设计原则（哲学）

### 风格 Sika Narrative（5 轮迭代总结）

> 违反其中任何一条，杂志感都会垮。

1. **克制优于炫技** — WebGL 背景只在 hero 页透出，普通页几乎看不见
2. **结构优于装饰** — 不用阴影、不用浮动卡片、不用 padding box，一切信息靠**大字号 + 字体对比 + 网格留白**
3. **内容层级由字号和字体共同定义** — 最大衬线 = 主标题，中衬线 = 副标，大非衬线 = lead，小非衬线 = body，等宽 = 元数据；Sika Red/Yellow 控制品牌层级
...
1. **Sika 固定锚点色** — 一份 deck 只用 Sika Red 作为 accent，不允许多色高亮拼贴；Sika Yellow 只承担品牌识别
...
本 skill 当前以 **Sika Corporate** 为唯一品牌口径。生成 deck 时优先参考以下语义,而不是重新套回通用杂志/通用网格主题:

- **Sika Red + Sika Yellow**:红色做结论、风险、KPI、章节强焦点；黄色做品牌条、角标、徽章和轻量现场标识。
- **建筑化学品 / 施工现场**:暖砂灰、直角网格、发丝线、编号、流程和材料质感要优先服务 Sika 的工业可信度。
- **Sika "Building Trust" 语义**:页面语言强调质量、可靠、工程、项目交付、可持续建造和长期信任。
- **Precision grid as structure**:Sika Precision 保留 精密网格结构,但颜色必须回到 Sika Corporate。
- **Editorial casebook as narrative**:Sika Narrative 保留杂志叙事,但纸张、标题和装饰必须使用 Sika 红黄暖底。
