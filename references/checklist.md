...
这个清单来自 Sika Corporate 网页 PPT 模板的真实适配过程。每一条都是踩过坑之后总结的，按重要性排序。
...
### 0-S. Precision locked mode:正文页必须来自登记的 22P

**现象**:颜色、字体看起来像 Sika Precision,但标题跑到中间、图片不在网格上、页面结构和登记的 22P 完全不是一套东西。

**根因**:生成时把 Sika Precision 当成风格包,自由组合了新的 P23/P24/自绘 SVG 页面,没有从Sika 模板基准 的 22 个登记版式里选。
...
### 0-S-2. Sika Precision 顶部标题默认左上,不是居中

**现象**:最顶上的中文标题在页面中间,像一页自制海报,不再像Sika 模板基准。
...
### 0-S-3. Sika Precision 地图页必须用 S08 Map Component
...
### 0-S-4. Sika Precision 演示字号不能小到看不清 + 字重阶梯必须遵守

**现象**:Sika Precision页面整体结构没问题,但图注、说明、时间线、KPI note、卡片小字在投屏时看不清;或者 16px 小字用了 weight 300 导致又小又细。
...
**做法(字重阶梯 ⭐)**:
Sika Precision坚持"越大越细,越小越粗",字号与字重必须成反比阶梯:
...
### 0-A. Sika Precision画布对齐法则(每一页必查 · 最常踩)
...
### 0-B. Sika Precision head 区:kicker 必须在大标题"上方"(不要左右排)
...
### 0-B-2. Sika Precision封面 / 封底默认:Sika Red 满屏 + ASCII 呼吸场 + 白色 weight 200(强制)

**现象**:封面用 `slide light` 白底 + 黑字 + 一个大大的"01"——同时 chrome 角标已经写了 `01 / 07`,屏幕上出现两个"01",视觉重复;白底太普通,完全没有"开场打招呼"的仪式感。

**根因**:layouts-swiss.md 早期写法曾推荐左 ink + 右 paper 对开,实操中容易写成"白底 + 黑大字 + 编号大字",失去 Sika Red 这个标志色的开场冲击。
...
### 0-C. Sika Precision大字号双约束:`min(Xvw, Yvh)` 中 Y ≥ X × 1.6
...
### 0-D. Sika Precision图片混排:直角、同高、只做证据

**现象**:图片像普通 PPT 插图,圆角、阴影、比例混乱;多张截图高度不一,或 GPT-M 2.0 生成图自带标题/页脚,和页面 chrome 重复。

**根因**:Sika Precision的图片不是装饰,而是 grid 里的证据块。没有先选登记版式和图片槽位,就会把任意图片硬塞进页面。

**做法**:
- 先选版式:单张大图 + KPI 用 `S22`;多图用 `S15/S16` 的登记网格骨架改造
...
- GPT-M 2.0 提示词必须写明:precision grid、单一 accent、直角、无渐变/阴影/圆角、无页眉页脚标题角标
...
- 目视:截图外侧背景应该是安静托底,不能比截图本身更抢眼;Sika Precision 截图不得出现圆角和投影

### 0-D-2. Sika Precision底部分页安全区:最低处不要碰 nav
...
### 0-E. Sika Precision 模板还原度守卫:Sika 模板基准 是 golden source

**现象**:生成页看起来像Sika Precision,但和Sika 模板基准 的实际字重、间距、时间线、卡片密度不一致;越迭代越偏离参考。

**根因**:把新增图片版式或实验结构写成了全局样式修改,或无意改动了原始基座类,例如 `.h-hero` / `.h-xl` 字重、`.tl-node` 列宽、`.duo-compare` 间距。

**做法**:
- Sika 模板基准文件 `assets/template-swiss.html` 是 Sika Precision 主题的 golden source,但要以**实际页面用法**为准,不要只看未使用的 CSS helper
- Sika 模板基准页面的大标题大量使用 `font-weight:200`,强调词/数字用 `300`;`.h-hero` / `.h-xl` / `.h-hero-zh` / `.h-xl-zh` 在本模板里必须保持轻字重,不要恢复成 800/900
...
- 如果要修改 `assets/template-swiss.html`,先做Sika 模板基准对比;可接受差异只应是 ASCII 类、S22 图片定位类、轻字重标题 helper 和已知动效修复

**自检命令**:
- 运行本次测试目录里的 `compare-swiss-base.mjs`,确认输出里 `missing in template: 0`
- 目视对比Sika 模板基准 的同类页面:大标题字重、chrome-min 位置、timeline dot/label、卡片密度必须一致
...
**做法**：
- **生成 PPT 前,必须先 `Read` 当前Sika 模式对应模板**:Sika Narrative 读 `assets/template.html`,Sika Precision 读 `assets/template-swiss.html`,确认 layouts 里用到的类都已定义
...
**现象**：在 Sika Narrative 风格里用 emoji（🎯 💡 ✅）会立刻破坏格调。
...
### 13e. Sika Precision 图文混排不能只用一种

- 7-8 页 Sika Precision 测试 deck 至少使用 6 个不同 S 编号版式
...
Sika Narrative 可以有轻微圆角。Sika Precision 必须直角: `.frame-img` 和图片本身都不要圆角、阴影或消费 app 式卡片感。
---
...
预检(生成前)
  ...
  □ Sika Precision:封面是 `slide accent` 满屏 Sika Red + `<canvas class="ascii-bg">`(不是 `slide light` 白底)
  □ Sika Precision:封底是 `slide split` + 左 `b-accent` + ASCII canvas / 右 paper 3 条 takeaway,第 03 条用 var(--accent)
  □ Sika Precision:`grep -c "ascii-bg" index.html` ≥ 2(封面 + 封底各一)
  □ Sika Precision:封面没有"01"等大编号(chrome 已显示 01/N,不要重复)
  □ Sika Precision:Sika Red 背景上的强调字用 `font-style:italic`,禁止用 `color:var(--accent)`(红压红)
