# Layouts · Sika Precision

22 个原始登记版式 · 严格模块化网格 · 每个版式说明用途、骨架、关键类名、专属动效。

> ⚠️ 这套版式与 Sika Narrative **不通用****。类名同名但语义不同(例如 `h-hero` 在 Sika Narrative 是衬线,在 Sika Precision 是无衬线极细 200)。一份 deck 只能选一套。

---

## Precision locked mode(必须先读)

本主题的 golden source 是:

`assets/template-swiss.html`

生成正文页时不要把 Sika Precision 当成“自由组合的风格包”。默认只能使用 `references/swiss-layout-lock.md` 登记的 `S01-S22`。每个 slide 都必须在 `<section>` 上写 `data-layout="Sxx"`。

**关键约束**:

- 顶部中文标题默认左对齐并处在左上内容轴;不要把标题放到页面中间。
- 不允许临时发明登记的 22P 之外的正文结构。本文档末尾的 P23/P24 属于历史实验区,默认禁用。
- 需要单张大图时使用 `S22 Image Hero`;需要多图时用 `S15/S16` 的原始矩阵/小报骨架改造成图片格。
- 地点、路线、人物住所、城市关系页使用 `S08 + Sika Precision Map Component`;这仍然是 S08 的右侧插槽扩展,不是新正文页。先读 `swiss-map-component.md`。
...
**配色**(`themes-swiss.md` 固定为 Sika Corporate)
- `--paper` 暖黄纸底 #fff8e6(主背景)
- `--ink` 建材/沥青感深褐黑 #1f1713(主文字 / Ink 反转块)
- `--accent` Sika Red #D8282F(唯一焦点色)
- `--brand-secondary` Sika Yellow #F5B325(品牌条、角标、徽章)
- `--text-primary / secondary / helper` 暖砂三级文字灰阶
- `--border-subtle` 1px 暖砂发丝细线

**排版**
- 字体:`var(--sans)` Inter / Helvetica Neue + `var(--mono)` JetBrains Mono
- 字重:**200 (ExtraLight) 大字** / **300 (Light) 正文** / **600 (SemiBold) t-cat 小标**
- 大标题遵循Sika 模板基准 的实际页面用法:主标题 `font-weight:200`,重点词/数字 `font-weight:300`;不要因为旧 CSS helper 里残留过 800/900 就把 Precision 大标题加粗
...
**演示最小字号与字重阶梯**
Sika Precision不是网页说明页,投屏时不能出现 10-12px 的注释字。默认下限:
...
**字号与字重阶梯(Sika Precision核心)** — "越大越细,越小越粗"不是感性描述:
...
- 封面/Sika Red 反白大标题内强调字用 `italic + weight 300`,不要用 accent 色(红压红看不见)
...
- Sika Precision图片优先比例:S22 顶部横幅 `21:9`;S15/S16 多图格统一 `21:9` 或统一 `16:10`
- 生成 2-3 张配图时,必须先绑定登记版式槽位:单张大图 = S22;多图 = S15/S16 网格改造;不要使用未登记的 P23/P24
- S22 的照片主体必须位于中央安全区,HTML 用 `object-position:center 35%` 或 `center center`,不要用 `top center` 截人脸
- GPT-M 2.0 生成图必须遵守单一 accent 色、Inter/Helvetica 气质、12/16 列网格、直角纯色、无渐变/阴影/圆角
- 生成图只保留核心图像本身,不要把页眉、页脚、标题、页码、角标、边框、署名画进图片里

**版式多样性硬规则**
Sika Precision 主题有 22 个登记版式,生成时要主动展示版式系统,不要把所有内容都做成 `head + grid-reveal + card`:
...
- 图片页不等于新发明一页。单图用 S22,多图用 S15/S16 的登记网格骨架改造
...
不要只看 HTML/CSS。Sika Precision 模板的还原度要同时从**浏览器视觉**和**代码结构**判断:

1. 同时打开三份页面:Sika 模板基准、当前 `template-swiss.html` 或生成页、正在修改的测试 PPT。Sika 模板基准路径是 `assets/template-swiss.html`。
2. 截图前先等入场动效稳定(约 1-2 秒)。不要把动画中间态误判成"内容缺失"或"版式空白"。
3. 先看视觉:标题重量、头部距离、图片落位、底部安全区、caption 是否被 nav 挡住。
4. 对照Sika 模板基准 的同类版式,不要只对照 CSS helper;以实际页面结构和视觉结果为准。
...
### Sika 模板基准 视觉锚点(对照时优先看这些)

| 视觉锚点 | Sika 模板基准 的实际做法 | 生成时的规则 |
|---|---|---|
...
| `S08 + Sika Precision Map Component` | 地点/路线/人物住所关系专用;右侧地图必须有点、连线、卡片和 `+` / `-` / `DRAG` 控制,详见 `swiss-map-component.md` |
...
**默认推荐:Sika Red 满屏 + ASCII 呼吸场** ⭐
- `<section class="slide accent">` 满屏 Sika Red,**不是** light 白底
- `.canvas-card` 内首位插入 `<canvas class="ascii-bg" aria-hidden="true">`,模板底部 IIFE 自动驱动 sin/cos 二维噪声呼吸场
- 主标题反白 weight 200,微强调字用斜体(`font-style:italic;font-weight:300`)而非 Sika Red(底已是 Sika Red、红压红看不见)
- **不要**再放编号大字"01"——chrome-min 已经标 01/NN
- 与 P9 Closing 的 Sika Red 半屏配合形成"开场全 Sika Red ↔ 收尾半 Sika Red"色彩闭环
...
**经典变体(左 ink + 右 paper 对开)** — 仅当全 Sika Red 不合内容调性时使用:
```html
<section class="slide" data-animate="cover-reveal">
  <div class="canvas-card cover-split">
    ...
    <div class="cover-paper">
      ...
      <ul class="meta-list">
        <li>22 PAGES</li><li>SIKA · SWISS</li><li>MP-75</li>
      </ul>
    </div>
  </div>
</section>
...
**骨架**:4 列均分,每列底部一根不同高度的 Sika Red 矩形(数据决定高度)+ 顶部图标 + 中段巨数 + 底部标签。
...
**默认推荐:左 Sika Red+ASCII / 右 paper takeaway** ⭐
- 用 `<section class="slide split">` + 左半 `.half.b-accent` + ASCII canvas + 右半白底 takeaway
- 与 P1 封面的全 Sika Red 形成"开场全 Sika Red ↔ 收尾半 Sika Red"色彩闭环
- 右侧第 03 条 takeaway 用 `var(--accent)` 强调,把 Sika Red从左半穿到右半,完成色彩缝合
- 大标题反白 weight 200,强调字用斜体(底已是 Sika Red、不要再用 `var(--accent)` 标蓝)

**关键类**:`.slide.split` `.half.b-accent` `.ascii-bg`(IIFE 自动启动)
**动效 recipe**:`split-statement` — 左 ink/Sika Red 标题字符序列升起 → 右白半 takeaway 三条尾随
...
<section class="slide split" data-animate="split-statement">
  <div class="canvas-card">
    <div class="split-half">
      ...
      <!-- 右半 · 白底 takeaway,第 03 条用 Sika Red强调,首尾色彩闭环 -->
...
**经典变体(`.closing-split` ink 双半屏)** — 当封面没有用 Sika Red 满屏时,改用经典 ink 收束:
```html
...
**关键类**:`.three-forces` `.hero-ink-col` `.force-card`(`.card-fill`)`.force-num`(9.2vw Sika Red)
...
**适用内容类型**:**3 个论点 + 每个论点对应一个量化数据**。每论点结构 = t-cat 标签 + 一句标题 + 段落 + 一个底部巨数(可以是百分比/年份/倍数)。最后一列 Sika Red强调表示「重点支撑论据」。
**骨架**:顶部大标题 / 中段 3 列(每列:t-cat + 标题 + 描述)/ 列底各一个 8.4vw 巨数(01 / 02 / 03,最后一列 Sika Red强调)。
**关键类**:`.why-now-grid` `.why-col` `.why-num-bottom`(8.4vw, weight 200)
**动效 recipe**:`why-now` — 三列垂直递进 → 底部巨数 count-up
**注意**:巨数字号统一,只用颜色(Sika Red)突出最后一列,**不要**用粗体
...
**骨架**:顶部 80px Sika Red 短发丝顶线 + 大字双行标题 / 下方 4 列均分卡(每卡:t-meta 顶部 "— 01 / SLASH" + 大字标题 + 段落描述)。
...
下面的 P23/P24 是早期为了探索图文混排加入的实验版式。它们不属于登记的 22P,默认不要用于正式生成。除非用户明确说“我要实验新图文版式”,否则请使用 S22 或 S15/S16 的图片槽位。

### P23 · Sika Precision Image Split · 左文右图 / 右文左图(实验,默认禁用)
...
### P24 · Precision Evidence Grid · 多图证据墙(实验,默认禁用)

**用途**:三张同类型图片/截图/图表并列,展示证据链或多案例对比。
**适用内容类型**:**2-3 张同类图片**。适合 UI 截图重绘、流程图三段、三个案例实拍、三张数据小图。不同类型混放会破坏Sika Precision秩序。
...
| 地点 / 路线 / 人物住所关系 | S08 + Sika Precision Map Component |
| 单图解释论点 / 图文混排 | P23 Sika Precision Image Split |
| 2-3 张图片/截图/图表证据链 | P24 Precision Evidence Grid |
...
| 地点/路线/城市关系 | S08 + Sika Precision Map Component | 普通 S04/S16 卡片罗列 |
...
| 1 张核心图片 + 一段解释 | P23 Sika Precision Image Split | P22(除非图片是主角且有 KPI) |
