...
| `inset` | `none` / `subtle` / `balanced` | 截图需要从背景中浮出来时用 balanced;Sika Precision 多用 none/subtle |
| `shadow` | `none` / `soft` / `editorial` | Sika Narrative 可 soft/editorial;Sika Precision 默认 none |
| `corners` | `square` / `small` / `medium` | Sika Precision square;Sika Narrative small/medium |
...
本仓库当前按 Sika Corporate 统一截图背景。内置 WebP 背景资产可以继续作为纹理底图使用,但最终合成必须叠加 Sika Red / Sika Yellow / 暖砂灰的统一视觉,不要让非 Sika 主题色成为页面主视觉。

| 用途 | 推荐资产 | Sika 处理方式 |
|---|---|---|
| Sika Narrative 截图 | `assets/screenshot-backgrounds/sika-narrative/kraft-paper.webp` 或 `dune.webp` | 叠加 `#fff8e6` 暖黄纸底,用 `#D8282F` 做 1px 顶线或角标。 |
| Sika Precision 截图 | `assets/screenshot-backgrounds/sika-precision/lemon-grid.webp` 或 `safety-orange-halftone.webp` | 降低原图饱和度,叠加暖砂灰,只保留 Sika Red 小面积线条/编号。 |
...
| 代码截图 | Sika Narrative 用纸感背景;Sika Precision 用浅网格背景;文字必须可读 |
