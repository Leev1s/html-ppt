# Sika Precision Map Component

用于 Sika 项目地点、施工路线、工厂/仓库/渠道分布、城市案例和事件点位等内容。它不是新的 Precision 正文版式,而是 **S08 Duo Compare 的右侧插槽扩展**:左侧仍是解释卡片,右侧替换为地图组件,配色必须继承 Sika Red / Sika Yellow / 暖砂灰。
...
- 重点点位用 Sika Red (`var(--accent)`),辅助区域和控制条可使用 Sika Yellow (`var(--brand-secondary)`)。
- 地图卡片底色保持暖砂灰/暖白,不要引入第三方品牌色。
- 地图 label 和关系线必须服务 Sika 项目叙事,例如施工点、供应链节点、渠道覆盖、项目进度。
...
(() => {
  ...
  window.addEventListener('DOMContentLoaded', () => setTimeout(initPrecisionMap, 500));
})();
</script>
```
