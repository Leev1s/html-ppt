# Sika 瑞士国际主义主题（Swiss Themes）

本仓库当前是 **Sika 专用网页 PPT 模板**。Style B（瑞士国际主义）保留 16 列网格、发丝线、直角色块和极致字号对比，但配色固定为 **Sika Corporate**，不再推荐任何通用瑞士主题或其他品牌色。
...
| `.sika-corner` | 右上角红黄几何角标，默认用于 Swiss cover。 |
| `.sika-badge` | 倾斜 Sika 字样徽章，适合 chrome/meta 区，不替代官方 logo。 |
| `.sika-stripe` | 小型品牌色条，适合 KPI 组、图注、卡片顶部。 |
| `.slide.sika-yellow` | 黄色主底页面，适合轻量转场，不建议承载长正文。 |

---

## 切换原则

- **一份 deck 固定 Sika Corporate**。不要中途换成通用瑞士主题或其他品牌色。
- **红色负责焦点，黄色负责识别**。如果页面显得太“红”，降低红色面积，而不是引入第三种颜色。
...
- ❌ 不要把通用 Swiss 主题重新引回 Sika deck。
- ❌ 不要接受用户给的任意 hex 值；只能在 Sika Red / Sika Yellow / 暖砂灰阶内调整面积和层级。
- ❌ 不要用渐变、阴影、圆角卡片弱化瑞士风结构；Sika 风也保持直角、发丝线、网格和纯色色块。
- ❌ 不要仿制、拉伸或重画官方 logo；模板只提供排版化的 Sika 识别元素。

生成完成后在项目记录中写明：`Theme: Sika Corporate Swiss · Accent #D8282F · Secondary #F5B325`。
