# Sika Narrative 主题（Themes）

本仓库当前是 **Sika 专用网页 PPT 模板**。Sika Narrative（Sika editorial casebook）不再提供多套可选色板；默认且唯一推荐的主题是 **Sika Corporate**：Sika Red 作为标题/深色页墨色，低饱和 Sika Yellow 作为纸张底色，形成企业案例手册、施工现场指示牌和建筑化学品资料页的统一气质。
...
| `.slide.sika-yellow` | 黄色主底页面，适合过渡页、封面反转或轻量提示页。 |
| `.sika-yellow-panel` | Sika Narrative 黄底页卡片,`.red` 变体用于唯一红色焦点卡。 |
...
- **黄底可以作为反转模板使用**,但只用于封面、章节过渡、KPI 或行动号召;正文长页仍优先使用 `--paper` / `--paper-tint`。
